...
class BuildUnit:
    ...
    #
    def __str__(self):
        return str(self.BuildObject) + str(self.CovcFlag)
    ...
    def __eq__(self, Other):
        return Other != None and self.BuildObject == Other.BuildObject \
                ...
                and self.CovcFlag == Other.CovcFlag
    ...
    #
    def __hash__(self):
        return hash(self.BuildObject) + hash(self.BuildObject.Arch) + hash(self.CovcFlag)

    def __repr__(self):
        return repr(self.BuildObject) + repr(self.CovcFlag)
...
class ModuleMakeUnit(BuildUnit):
    ...
    #   @param  Target      The build target name, one of gSupportedTarget
    #
    def __init__(self, Obj, Target, CovcFlag):
        Dependency = [ModuleMakeUnit(La, Target, CovcFlag) for La in Obj.LibraryAutoGenList]
        if Obj.IsLibrary and Obj._BuildDir2 != None and CovcFlag:
            BuildUnit.__init__(self, Obj, Obj.BuildCommand, Target, Dependency, Obj.MakeFileDir.replace (Obj._BuildDir, Obj._BuildDir2))
        else:
            BuildUnit.__init__(self, Obj, Obj.BuildCommand, Target, Dependency, Obj.MakeFileDir)
        if Target in [None, "", "all"]:
            self.Target = "tbuild"
        self.CovcFlag = CovcFlag
...
class BuildTask:
    ...
    def AddDependency(self, Dependency):
        for Dep in Dependency:
            if True: #not Dep.BuildObject.IsBinaryModule:
            ...
            for ToolChain in self.ToolChainList:
                ...
                for Arch in Wa.ArchList:
                    ...
                    for Ma in self.BuildModules:
                        CovcFlag = False
                        if GlobalData.gCodeCov != None:
                            ModuleStr = os.path.normpath(str(Ma.Module).replace(Ma.WorkspaceDir, ''))
                            if ModuleStr[0] == '\\':
                                ModuleStr = ModuleStr[1:]
                            CovcFlag = GlobalData.gCodeCov.AssertModule(ModuleStr)
                        # Generate build task for the module
                        if not Ma.IsBinaryModule:
                            Bt = BuildTask.New(ModuleMakeUnit(Ma, self.Target, CovcFlag))
...
def MyOptionParser():
    ...
    Parser.add_option("-E", "--en-codecov", action="callback", type="string", dest="Covfile", callback=SingleCheckCallback, 
        help="Enabled the code coverage and specified the module config file.")
    Parser.add_option("-Z", "--zero-covfile", action="store_true", dest="ZeroCovFile", default=False, help="Zero the code coverage data file.")
...
def Main():
    ...
    EdkLogger.quiet("##########################################################################################\n")
    EdkLogger.quiet("############# Enhanced Build Tools for Code Coverage Analysis Build r16814   ##############\n")
    EdkLogger.quiet("##########################################################################################\n")
    ...
    try:
        ...
        if Option.Covfile != None:
            #No allow the multi-thread build for code coverage now. Need to be fixed.
            Option.ThreadNumber = 1
            if os.path.isabs (Option.Covfile):
                if os.environ['WORKSPACE'] in Option.Covfile:
                    Option.Covfile = Option.Covfile.replace(os.environ['WORKSPACE'], '')
                    if Option.Covfile == '\\':
                        Option.Covfile = Option.Covfile[1]
                if os.path.normcase (os.path.normpath(Option.Covfile)).find (Workspace) == 0:
                    Option.PlatformFile = NormFile(os.path.normpath(Option.Covfile), Workspace)
            Option.Covfile = PathClass(Option.Covfile, Workspace)
            GlobalData.gCodeCov = Utils.CovParser(Option.Covfile.Path)
            DefineList          = GlobalData.gCodeCov.GetDefineList()
            os.environ['COVC']         = DefineList['COVC']
            os.environ['CovCofigFile'] = Option.Covfile.Path
            EdkLogger.quiet("%-16s = %s" % ("COVCONFIGINI", Option.Covfile.Path))
            # Generate Config file 
            try:
                covucda = os.path.join(os.environ['WORKSPACE'], DefineList['UCDAFILE'])
                if Option.ZeroCovFile == True:
                    f = open(covucda, 'w')
                    f.close()
                os.environ['UCDAFILE']  = covucda
                os.environ['COVFILE']   = covucda
                EdkLogger.quiet("%-16s = %s" % ("UCDAFILE", covucda))
            except:
                pass
            
            try:
                covucna = os.path.join(os.environ['WORKSPACE'], DefineList['UCNOFILE'])
                if Option.ZeroCovFile == True:
                    f = open(covucna, 'w')
                    f.close()
                os.environ['UCNOFILE']  = covucna
                EdkLogger.quiet("%-16s = %s" % ("UCNOFILE", covucna))
            except:
                pass
            
            covfile = os.path.join(os.environ['WORKSPACE'], DefineList['COVCCFG'])
            f = open(covfile, 'w')
            os.environ['COVCCFG'] = covfile
            EdkLogger.quiet("%-16s = %s" % ("COVCCFG", covfile))
            SourceList = GlobalData.gCodeCov.GetSourceList()
            for SourceStr in SourceList.keys():
                FullPath = os.path.join (Workspace, SourceStr)
                if SourceList[SourceStr]:
                    StrLine = '-t' + FullPath + '\n'
                    ...
                    f.writelines(StrLine)
            f.close()
            os.environ['ITS_BUILD']         = '1'
            try:
                ItsToolChan = os.environ['ITSCOVTOOLCHAIN'] 
            except:
                ItsToolChan = ''
            if ItsToolChan == '':
                if Option.ToolChain != None:
                    if 'VS2008' in Option.ToolChain:
                        os.environ['ITSCOVTOOLCHAIN'] = 'VS2008'
                    elif 'VS2010' in Option.ToolChain:
                        os.environ['ITSCOVTOOLCHAIN'] = 'VS2010'
                    elif 'VS2012' in Option.ToolChain: 
                        os.environ['ITSCOVTOOLCHAIN'] = 'VS2012'
                    elif 'VS2013' in Option.ToolChain: 
                        os.environ['ITSCOVTOOLCHAIN'] = 'VS2013'
                    elif 'VS2014' in Option.ToolChain: 
                        os.environ['ITSCOVTOOLCHAIN'] = 'VS2014'
                    elif 'VS2015' in Option.ToolChain: 
                        os.environ['ITSCOVTOOLCHAIN'] = 'VS2015'
                    else:
                        EdkLogger.error("build", OPTION_VALUE_INVALID, "Please set ITSCOVTOOLCHAIN variable of " + os.environ['COVFILE'] + " in command line.")
                else:
                    EdkLogger.error("build", OPTION_VALUE_INVALID, "Please set ITSCOVTOOLCHAIN variable of " + os.environ['COVFILE'] + " in command line.")
