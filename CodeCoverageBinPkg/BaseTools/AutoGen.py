...
class ModuleAutoGen(AutoGen):
    ...
    def _GetBuildDir(self):
        if self._BuildDir == None:  
             if os.environ['COVC']:
                 # For code coverage build
                 ModuleList = GlobalData.gCodeCov.GetModuleList()
                 ModuleStr = os.path.normpath(str(self.Module).replace(self.WorkspaceDir, ''))
                 if self._IsLibrary():
                 # For library, needs to create two AutoGen codes in two different directories
                     self._BuildDir = path.join(
                     ...
                     CreateDirectory(self._BuildDir2)
                 else:
                     # For module, only need to create one AutoGen code
                     if ModuleStr[0] == '\\':
                         ModuleStr = ModuleStr[1:]
                     if GlobalData.gCodeCov.AssertModule(ModuleStr):
                         self._BuildDir = path.join(
                                         self.PlatformInfo.BuildDir,
                                         self.Arch,
                                         'COVC',
                         ...
                         self._BuildDir2 = self._BuildDir
                     else:
                         self._BuildDir = path.join(
                                         self.PlatformInfo.BuildDir,
                                         self.Arch,
    ...
    def _GetOutputDir(self):
        if self._OutputDir == None:
            ...
            if self._BuildDir2 != None:
                CreateDirectory(path.join(self._BuildDir2, "OUTPUT"))
    ...
    def _GetDebugDir(self):
        if self._DebugDir == None:
            ...
            if self._BuildDir2 != None:
                CreateDirectory(path.join(self._BuildDir2, "DEBUG"))
    ...
    def _ApplyBuildRule(self, File, FileType):
        ...
        while Index < len(SourceList):
            ...
            if Source != File:
                ...
                if self.IsLibrary and self._BuildDir2 != None:
                    CreateDirectory(Source.Dir.replace (self._BuildDir, self._BuildDir2))
    ...
    def CreateMakeFile(self, CreateLibraryMakeFile=True):
        ...
        if self.IsMakeFileCreated:
            return
        if not self.IsLibrary and CreateLibraryMakeFile and not self.IsBinaryModule:
        ...
        try:
            Result = Makefile.Generate()
        except: 
            return
        #
        # Replace the code coverage flag and runtime library
        #
        if GlobalData.gCodeCov != None:
            DefineList = GlobalData.gCodeCov.GetDefineList()
            if self.IsLibrary:
                covcstr   = DefineList['COVC']
                covlibstr = ''
                MakefilePath = os.path.join(Makefile._AutoGenObject.MakeFileDir, 'Makefile')
                import shutil
                #Build the library with no code coverage
                shutil.copyfile (MakefilePath, MakefilePath.replace(self._BuildDir, self._BuildDir2))
                MakefilePath = os.path.join(Makefile._AutoGenObject.MakeFileDir, 'Makefile').replace(self._BuildDir, self._BuildDir2)
                BinDir = ''
                NewBinDir = ''
            else:
                SecType  = ['SEC']
                PeiType  = ['PEI_CORE', 'PEIM']
                DxeType  = ['DXE_CORE', 'DXE_DRIVER', 'DXE_SAL_DRIVER', 'UEFI_DRIVER', 'UEFI_APPLICATION']
                SmmType  = ['SMM_CORE', 'DXE_SMM_DRIVER']
                RtType   = ['DXE_RUNTIME_DRIVER']
                CovStr   = 'EDK2_'
                if self.Macros['MODULE_TYPE'] in SecType:
                    CovStr = CovStr + 'SEC'
                elif self.Macros['MODULE_TYPE'] in PeiType:
                    CovStr = CovStr + 'PEI'
                elif self.Macros['MODULE_TYPE'] in DxeType:
                    CovStr = CovStr + 'DXE'
                elif self.Macros['MODULE_TYPE'] in SmmType:
                    CovStr = CovStr + 'SMM'
                elif self.Macros['MODULE_TYPE'] in RtType:
                    CovStr = CovStr + 'RUNTIME'
                else:
                    CovStr = CovStr + self.Macros['MODULE_TYPE']
                if self.Macros['ARCH'] == 'IA32':
                    CovStr = CovStr + '_COVLIB_IA32'
                elif self.Macros['ARCH'] == 'X64':
                    CovStr = CovStr + '_COVLIB_X64'
                else:
                    EdkLogger.error("build", PARAMETER_INVALID,
                            ExtraData = "Code Coverage met a no-supported Arch %s" % 
                            (self.Macros['ARCH']))
                ModuleStr = os.path.normpath(str(self.Module).replace(self.WorkspaceDir, ''))
                if ModuleStr[0] == '\\':
                    ModuleStr = ModuleStr[1:]
                if GlobalData.gCodeCov.AssertModule(ModuleStr):
                    covcstr   = DefineList['COVC'] 
                    try:
                        covliblist = DefineList[CovStr].split(';')
                    except:
                        covliblist = []
                        EdkLogger.error("build", PARAMETER_INVALID,
                            ExtraData = "Code Coverage met a no-defined MACRO %s used in %s, please define it in config file." % \
                            (CovStr, str(self._Module)))
                    covlibstr = ''
                    for covlib in covliblist:
                        if covlib != '':
                            covlib = os.path.normpath(covlib)
                            covlibstr = covlibstr + ' ' + covlib
                else:
                    covcstr = ''
                    covlibstr = ''
                MakefilePath = os.path.join(Makefile._AutoGenObject.MakeFileDir, 'Makefile')
                BinDir = os.path.join('BIN_DIR = $(BUILD_DIR)', self.Arch)
                if covcstr != '':
                    NewBinDir = os.path.join(BinDir, 'COVC')
                else:
                    NewBinDir = BinDir
            fp=open(MakefilePath,'r') 
            alllines=fp.readlines()
            fp.close()
            fp=open(MakefilePath,'w') 
            for eachline in alllines:
                if self._BuildDir2 != None:
                    newline = eachline.replace(self._BuildDir, self._BuildDir2)
                else:
                    newline = eachline
                newline = newline.replace(BinDir, NewBinDir)
                newcovc = newline.replace('\"$(CC)\"', covcstr + ' ' + '\"$(CC)\"' )
                newcovrt = newcovc.replace('$(DLINK_SPATH)', '$(DLINK_SPATH)' + ' ' + covlibstr)
                fp.writelines(newcovrt)
                if self._BuildDir2 != None:
                    # Add the CP command to copy the .efi to the old build path for compatibility
                    # $(CP) * $(BIN_DIR)
                    if '$(CP)' in newline and '$(BIN_DIR)' in newline:
                        newcovrt = newline.replace('$(BIN_DIR)', '$(BUILD_DIR)\\$(ARCH)')
                        fp.writelines(newcovrt)
    ...
    def CreateCodeFile(self, CreateLibraryCodeFile=True):
        ...
        for File in self.AutoGenFileList:
            if GenC.Generate(File.Path, self.AutoGenFileList[File], File.IsBinary):
                ...
                if GlobalData.gCodeCov == None and self.AutoGenVersion < 0x00010005 and File.Name == 'AutoGen.c':
                    continue

                AutoGenList.append(str(File))
            else:
                IgoredAutoGenList.append(str(File))
            if self.IsLibrary and GlobalData.gCodeCov != None:
                CovFilePath = File.Path.replace(self._BuildDir, self._BuildDir2)
                GenC.Generate(CovFilePath, self.AutoGenFileList[File], File.IsBinary)
            if not self.IsLibrary and GlobalData.gCodeCov != None and GlobalData.gCodeCov.AssertModule(self.Module.MetaFile.File):
                CovFilePath = File.Path.replace(self._BuildDir, self._BuildDir2)
                GenC.Generate(CovFilePath, self.AutoGenFileList[File], File.IsBinary)
        # Skip the following code for EDK I inf
        if self.AutoGenVersion < 0x00010005:
