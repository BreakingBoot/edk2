...
\t${END}@cd $(MODULE_BUILD_DIR)\n\n''')
    ...
    def _CreateTemplateDict(self):
        ...
        if self._AutoGenObject.Arch == "EBC":
            ...
            if GlobalData.gCodeCov != None and GlobalData.gCodeCov.AssertModule(str(self._AutoGenObject.MetaFile.File)) and \
            not self._AutoGenObject.IsLibrary and self._AutoGenObject.Module.ModuleType in \
            ["DXE_DRIVER", "DXE_SMM_DRIVER", "DXE_RUNTIME_DRIVER", "DXE_SAL_DRIVER", "UEFI_DRIVER", "UEFI_APPLICATION", "SMM_CORE"]:
                ImageEntryPoint = "ProcessModuleEntryPointList"
            else:
        ...
        for ListFileMacro in self.ListFileMacros:
            ...
            ListFileMacros = []
            # Get the COVC base path
            if self._AutoGenObject._BuildDir2 != None:
                Index = self._AutoGenObject._BuildDir2.find ('COVC')
                BuildDir  = self._AutoGenObject._BuildDir2[:Index]
                BuildDir2 = self._AutoGenObject._BuildDir2[:Index + 5]
            else:
                BuildDir = self._AutoGenObject._BuildDir
                BuildDir2 = self._AutoGenObject._BuildDir2
                
            if BuildDir2 != None:
                for Macro in self.ListFileMacros[ListFileMacro]:
                    Macro = os.path.normpath(Macro)
                    if os.path.normpath(BuildDir2) in os.path.normpath(Macro):
                        ListFileMacros.append(Macro)
                    else:
                        ListFileMacros.append(Macro.replace(BuildDir, BuildDir2))
                # For library, generate two object_files.list if COVC is enabled.
                if self._AutoGenObject.IsLibrary:
                    SaveFileOnChange(
                                ListFileName.replace(BuildDir, BuildDir2),
                                "\n".join(ListFileMacros),
                                False
                                )
            # For library and module, replace the COVC path
            if self._AutoGenObject.IsLibrary:
                SaveFileOnChange(
                    ListFileName,
            ...
            if not self._AutoGenObject.IsLibrary and BuildDir2 != None:
                SaveFileOnChange(
                    ListFileName,
                    "\n".join(ListFileMacros),
                    False
                    )
            elif not self._AutoGenObject.IsLibrary and BuildDir2 == None:
                SaveFileOnChange(
                    ListFileName,
                    "\n".join(self.ListFileMacros[ListFileMacro]),
                    False
