...
## The class implementing the code covearage config file parser
#
class CovParser():
    ##Return the module list of code coverage
    def __init__(self, CovfilePath):
        self._DefineDict         = None
        self._ModuleList         = None
        self._SourceList         = None
        self._DefineAutoGenDict  = None
        self.__DefineList     = []
        self.__ComponentsList = []
        self.__SourceList     = []
        self.__SourceTrue     = False
        self.__SourceFalse    = False
        try:
            fp=open(CovfilePath,'r') 
        except:
            EdkLogger.error("build", FILE_NOT_FOUND, ExtraData=CovfilePath)
        ...
        for eachline in alllines:
            ...
            if eachline == '' or eachline[0] == '#':
                continue
            if eachline.find('[Defines]') != -1:
                Flag = 'Define'
                continue
            if eachline.find('[Components]') != -1:
                Flag = 'Components'
                continue
            if eachline.find('[Source]') != -1:
                Flag = 'Source'
                continue
            if Flag == 'Define':
                self.__DefineList.append (eachline)
            elif Flag == 'Components':
                self.__ComponentsList.append (eachline)
            elif Flag == 'Source':
                self.__SourceList.append (eachline)
        fp.close()
         
    def __ReplaceMacros(self, Str):
        # Replace the $()
        MacroEnd = 0
        ...
        while Str.find('%', MacroEnd) >= 0:
            MacroStart = Str.find('%', MacroEnd)
            if Str.find('%', MacroStart) > 0:
                MacroEnd = Str.find('%', MacroStart+1)
                Name = Str[MacroStart + 1 : MacroEnd]
                Value = None
                try:
                    Value = os.environ[Name]
                except:
    ...
    def GetDefineList(self):
        if self._DefineDict == None:
            self._DefineDict = dict()
            for DefineStr in self.__DefineList:
                DefineStr = self.__ReplaceMacros(DefineStr)
                Name, Value = DefineStr.split('=')
                Name = Name.rstrip(' ')
                Value = Value.lstrip(' ')
                self._DefineDict[Name] = Value
        return self._DefineDict
    
    def GetAutoGenIncludeFile(self, ModulePath):
        if self._DefineAutoGenDict == None:
            self._DefineAutoGenDict = dict()
            if self._DefineDict == None:
                self.GetDefineList()
            Include = ''
            InfPath = ''
            for Name in self._DefineDict.keys():
                if 'AUTOGEN_H_INCLUDE_MAP' in Name:
                    Include, ReturnValue, InfPath = self._DefineDict[Name].split('|')
                    Include = Include.rstrip(' ')
                    InfPath = InfPath.lstrip(' ')
                    InfList = InfPath.split(';')
                    for Inf in InfList:
                        self._DefineAutoGenDict[Inf] = [Include, ReturnValue]
        if ModulePath in self._DefineAutoGenDict.keys():
            return self._DefineAutoGenDict[ModulePath]
        return [None, None]
    
    def GetModuleList(self):
        if self._ModuleList == None:
            self._ModuleList = dict()
            for ModuleStr in self.__ComponentsList:
                ModuleStr = self.__ReplaceMacros(ModuleStr)
                ModuleStr = os.path.normpath(ModuleStr)
                if ModuleStr[0] == '!':
                    self._ModuleList[ModuleStr[1:]] = False
                    self.__SourceFalse              = True 
                else:
                    if ModuleStr[1:] in self._ModuleList and\
                        self._ModuleList[ModuleStr[1:]] == False:
                        pass
                    else:
                        self._ModuleList[ModuleStr[0:]] = True
                    self.__SourceTrue               = True
        return self._ModuleList  
    
    def GetSourceList(self):
        if self._SourceList == None:
            self._SourceList = dict()
            for SourceStr in self.__SourceList:
                SourceStr = self.__ReplaceMacros(SourceStr)
                SourceStr = os.path.normpath(SourceStr)
                if SourceStr[0] == '!':
                    self._SourceList[SourceStr[1:]] = False
                else:
                    self._SourceList[SourceStr[0:]] = True
        return self._SourceList   
    
    def AssertModule (self, RelativeModulePath):
        if self._ModuleList == None:
            self._ModuleList = self.GetModuleList()
        ModuleStr = os.path.normpath(RelativeModulePath)
        
        # Only support the INF file
        if '.inf' not in RelativeModulePath:
            return False
        # Rule 1.If only list traced modules in this section, other modules will not be traced.
        if self.__SourceTrue  == True and self.__SourceFalse == False:
            if (ModuleStr in self._ModuleList.keys() and self._ModuleList[ModuleStr] == True):
                return True
            else:
                return False
        # Rule 2.If only list no-traced modules in this section, other modules will be traced.
        if self.__SourceTrue  == False and self.__SourceFalse == True:
            if (ModuleStr in self._ModuleList.keys() and self._ModuleList[ModuleStr] == False):
                return False
            else:
                return True 
        # Rule 3.If list no-traced and traced modules in this section, other modules will not be traced.
        #        If a module was listed more than one time and the status conflict, it will not be traced.
        if self.__SourceTrue  == True and self.__SourceFalse == True:
            if ModuleStr in self._ModuleList.keys():
                return self._ModuleList[ModuleStr]
            else:
                return False
