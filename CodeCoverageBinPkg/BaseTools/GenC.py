...
"""),
## Add for the EDKI code coverage
TemplateString("""
const UINT32 _gUefiDriverRevision = ${UefiSpecVersion};
const UINT32 _gDxeRevision = ${PiSpecVersion};
...
"""),
## Initialize the gST for EDK1 code coverage
TemplateString("""
const UINT32 _gUefiDriverRevision = ${UefiSpecVersion};
const UINT32 _gDxeRevision = ${PiSpecVersion};
...
"""),
# Add the blank function for EDK1 Code coverage
TemplateString("""
""")
]
...
def CreateModuleEntryPointCode(Info, AutoGenC, AutoGenH):
    ...
    if Info.ModuleType == 'PEI_CORE':
        ...
        if Info.AutoGenVersion < 0x00010005 and \
            GlobalData.gCodeCov != None and \
            GlobalData.gCodeCov.AssertModule(Info.Module.MetaFile.File):
            ReturnValue = GlobalData.gCodeCov.GetAutoGenIncludeFile (Info.Module.MetaFile.File)[1]
            if ReturnValue == 'VOID':
                AutoGenC.Append(gUefiDriverEntryPointString[-1].Replace(Dict))
                AutoGenH.Append(gUefiDriverEntryPointPrototype[-1].Replace(Dict))
            else:
                AutoGenC.Append(gUefiDriverEntryPointString[-2].Replace(Dict))
                AutoGenH.Append(gUefiDriverEntryPointPrototype[-2].Replace(Dict))
        else:
            if NumEntryPoints < 2:
                AutoGenC.Append(gUefiDriverEntryPointString[NumEntryPoints].Replace(Dict))
            else:
                AutoGenC.Append(gUefiDriverEntryPointString[2].Replace(Dict))
            AutoGenH.Append(gUefiDriverEntryPointPrototype[0].Replace(Dict))
        ...
        if Info.AutoGenVersion < 0x00010005 and \
            ...
            if ReturnValue == 'VOID':
                ...
                AutoGenH.Append(gDxeSmmEntryPointPrototype[-1].Replace(Dict))
            else:
                AutoGenC.Append(gDxeSmmEntryPointString[-2].Replace(Dict))
                AutoGenH.Append(gDxeSmmEntryPointPrototype[-2].Replace(Dict))
        else:
            if NumEntryPoints == 0:
                AutoGenC.Append(gDxeSmmEntryPointString[0].Replace(Dict))
            else:
                AutoGenC.Append(gDxeSmmEntryPointString[1].Replace(Dict))
            AutoGenH.Append(gDxeSmmEntryPointPrototype[0].Replace(Dict))    
        ...
        if Info.AutoGenVersion < 0x00010005 and \
            ...
            ReturnValue = GlobalData.gCodeCov.GetAutoGenIncludeFile (Info.Module.MetaFile.File)[1]
            if ReturnValue == 'VOID':
                AutoGenC.Append(gUefiApplicationEntryPointString[-1].Replace(Dict))
                AutoGenH.Append(gUefiApplicationEntryPointPrototype[-1].Replace(Dict))
            else:
                AutoGenC.Append(gUefiApplicationEntryPointString[-2].Replace(Dict))
                AutoGenH.Append(gUefiApplicationEntryPointPrototype[-2].Replace(Dict))
        else:
            if NumEntryPoints < 2:
                AutoGenC.Append(gUefiApplicationEntryPointString[NumEntryPoints].Replace(Dict))
            else:
                AutoGenC.Append(gUefiApplicationEntryPointString[2].Replace(Dict))
            AutoGenH.Append(gUefiApplicationEntryPointPrototype[0].Replace(Dict))
...
def CreateModuleUnloadImageCode(Info, AutoGenC, AutoGenH):
    ...
    if Info.AutoGenVersion < 0x00010005 and \
        ...
        GlobalData.gCodeCov.AssertModule(Info.Module.MetaFile.File):
        ReturnValue = GlobalData.gCodeCov.GetAutoGenIncludeFile (Info.Module.MetaFile.File)[1]
        AutoGenC.Append(gUefiUnloadImageString[-1].Replace(Dict))
    else:
        if NumUnloadImage < 2:
...
def CreateHeaderCode(Info, AutoGenC, AutoGenH):
    ...
    elif GlobalData.gCodeCov != None and \
        GlobalData.gCodeCov.AssertModule(Info.Module.MetaFile.File) and \
        not Info.IsLibrary:
        # C file header files includes
        if Info.ModuleType in ["DXE_DRIVER", "DXE_SMM_DRIVER", "DXE_RUNTIME_DRIVER", "DXE_SAL_DRIVER", "UEFI_DRIVER", "UEFI_APPLICATION", "SMM_CORE"]:
            Include = GlobalData.gCodeCov.GetAutoGenIncludeFile (Info.Module.MetaFile.File)[0]
            if Include == None:
                IncludeList = ['Tiano.h']
            else:
                IncludeList = Include.split(';')
            for IncludeStr in IncludeList: 
                AutoGenH.Append("#include \"%s\"\n" % (IncludeStr))
            AutoGenH.Append("EFI_SYSTEM_TABLE         *gST;")
    if Info.IsLibrary:
        return
    # C file header
...
def CreateCode(Info, AutoGenC, AutoGenH, StringH, UniGenCFlag, UniGenBinBuffer):
    ...
    if (GlobalData.gCodeCov != None and GlobalData.gCodeCov.AssertModule(Info.Module.MetaFile.File) and \
        not Info.IsLibrary and Info.ModuleType in ["DXE_DRIVER", "DXE_SMM_DRIVER", "DXE_RUNTIME_DRIVER", "DXE_SAL_DRIVER", "UEFI_DRIVER", "UEFI_APPLICATION", "SMM_CORE"]) or \
        Info.AutoGenVersion >= 0x00010005:
    ...
    if (GlobalData.gCodeCov == None  or (GlobalData.gCodeCov != None and not GlobalData.gCodeCov.AssertModule(Info.Module.MetaFile.File))) and \
        Info.AutoGenVersion < 0x00010005 and len(Info.UnicodeFileList) == 0:
        AutoGenC.String = ''
